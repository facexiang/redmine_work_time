class WorkTimeEntryController < ApplicationController
  unloadable
  ...
  def index
    ...
    if !params.key?(:user) then
      ...
      @my_projects = User.current.projects.active
      @my_projects_arr = User.current.projects.active.pluck(:id, :name)
      render "show"
    end
  end
  ...
  def ajax_relay_input # チケット選択の内容を返すアクション#返回选择票的内容的动作
  ...
  def change
    ...
    unless user.projects.active.exists?(id: project_id)
      render json: {stat: :error, msg: '未知项目！'} and return
    end
    hours = params[:val].to_f
    activity_id = TimeEntryActivity.last.id
    time_entry = WtTimeEntry.find_or_initialize_by(project_id: project_id, user_id: user.id, issue_id: nil, spent_on: Date.today)
    time_entry.activity_id = time_entry.try(:activity_id) || activity_id
    time_entry.hours = hours
    if time_entry.save
      render json: {stat: :ok, msg: :成功}
    else
      render json: {stat: :error, msg: time_entry.errors.full_messages[0]}
    end
  end
private
  def find_project
  ...
  def prepare_values
    ...
    @link_params = {:controller=>"work_time_entry", :id=>@project,
  ...
  def make_pack
    ...
    #dsp_prjs = Project.joins("INNER JOIN wt_project_orders ON wt_project_orders.dsp_prj=projects.id").
        ...
        #all
    dsp_prjs = User.current.projects.active
    dsp_prjs.each do |prj|
      ...
      make_pack_prj(@month_pack, prj, -1)
      make_pack_prj(@day_pack, prj, -1)
    end
    ...
    hours.each do |hour|
      ...
      if hour.issue && hour.issue.visible? then
        # 表示項目に工数のプロジェクトがあるかチェック→なければ項目追加 ##检查显示项目中是否有工程项目→如果没有项目追加
        prj_pack = make_pack_prj(@month_pack, hour.project)
        # 表示項目に工数のチケットがあるかチェック→なければ項目追加 ##检查显示项目是否有工时的票→如果没有项目追加
  ...
  def make_pack_prj(pack, new_prj, odr=NO_ORDER)
      ...
      ##检查显示项目是否有该项目→如果没有项目追加
      unless pack[:ref_prjs].has_key?(new_prj.id) then
        prj_pack = {:odr=>odr, :prj=>new_prj,
                    :total=>0, :total_by_day=>{},
                    :ref_issues=>{}, :odr_issues=>[], :count_issues=>0, :odr_time_entries=>[]}
  ...
  def make_pack_issue(prj_pack, new_issue, odr=NO_ORDER)
      ...
      # 表示項目に当該チケットがあるかチェック→なければ項目追加    检查显示项目是否有相应的票据→如果没有追加项目
      unless prj_pack[:ref_issues].has_key?(id) then
        ...
        if new_issue.nil?  # add nil issue
        ...
        prj_pack[:count_issues] += 1
        # change
        cnt_childrens = new_issue.nil? ? 0 : Issue.where(["parent_id = ?", new_issue.id.to_s]).count
